...
loginData = parse.urlencode([("email", "13934208183"),("password","123456789"),("chk","")])
...
def addSpiderData(data):
    ...
    cursor = db.cursor()
    for da in data:
        sql = "INSERT INTO table1(title,postTime,violiaTime,details,_clas) VALUES ('{}','{}','{}','{}','{}')".format(da['title'],da['postTime'],da['violiaTime'],da['details'],da['clas'])
        try:
            print('执行写入数据...')
            # 执行sql语句
            cursor.execute(sql)
...
def spider(pageOpen,pageLogin,loginData,pageSpider):
    # 获取cookie
...
if __name__ == '__main__':
    spider(pageOpen, pageLogin, loginData, pageSpider)
