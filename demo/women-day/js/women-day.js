(function(win, $) {
			...
			if (count < len) {
				...
				for (; i < len; i++) {
					if (data[count].answer[i]) {
						...
						ask.append('<div data-score="' + data[count].answer[i].score + '">' + data[count].answer[i].optionName + '</div>');
					}
				}
	...
	$(function() {
		...
		$('.page-foot').on('click', 'a', function(event) {
			...
			var index = $('.ask-warp').children('[data-score]').eq($(this).index());
			console.log(index);
			console.log(index.data('score'));
